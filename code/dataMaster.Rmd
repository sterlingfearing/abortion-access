...
```{r}
# add 1 to number of clinics in each county so proportions can be created
# 1=0

clinicCount$clinRoe <- clinicCount$clinRoe+1

clinicCount$clinDob <- clinicCount$clinDob+1
```
...
countyData <- join_county %>%
  select("GEOID", "stateFIPS.x", "countyFIPS.x", "county", "state", "popTot", "age15_49", "whiteP", "blackP", "amIndP", "asianP", "pacIsP", "hispP", "otherP", "povPerc", "pciE", "distRoe", "distDobb", "distDif", "distDifS", "ageD", "whiteD", "blackD", "asianD", "hispD", "raceD", "p100D", "p100_199D", "p200D", "incomeD", "totalD", "clinRoe", "clinDob", "geometry") %>%
...
dataComplete <- join_state_county %>%
  select(GEOID, stateFIPS, countyFIPS, county, state.x, popTot, age15_49, whiteP, blackP, amIndP, asianP, pacIsP, hispP, otherP, povPerc, pciE, clinRoe, clinDob, distRoe, distDobb, distDif, distDifS, ageD, whiteD, blackD, asianD, hispD, raceD, p100D, p100_199D, p200D, incomeD, totalD, legCon, govCon, stateCon, gestRoe, fundRoe, medRoe, persRoe, summRoe, gestDobb, fundDobb, medDobb, persDobb, summDobb, geometry) %>%
