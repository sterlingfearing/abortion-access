...
lawIndex <- lawRename %>%
   ...
   mutate(gestLimI = case_when(str_detect(gestLim, "6 weeks")~5,
                               str_detect(gestLim,"20 weeks")~4,
                               str_detect(gestLim, "24 weeks")~3,
                               str_detect(gestLim, "3rd trimester")~2,
...
# Modify values for exceptions/special characters
   # To do: record original values not NA when no special character is present
...
lawIndexM <- lawIndex %>%   
   mutate(gestLimI = gestLimI+case_when(str_detect(gestLim, "[‡]")~0.5, #life endangerment only
                                        ...
                                        str_detect(gestLim, "[Ω]")~-0.25, #exception:fetal abnormality
                                        str_detect(gestLim, "[*]")~-0.25)) %>% #exception:patient's phys health
   
   # To do: accommodate cases with 2 special characters, ie. "*, €" and "†, Ω"
                                       
                                        
   mutate(pbBanI = pbBanI+case_when(str_detect(pbBan, "[▼]")~0.25)) %>%  #law not in effect
   ...
   mutate(pubFundLimI = pubFundLimI+case_when(str_detect(pubFundLim, "[ϴ]")~0.25, #against court order
                                              ...
                                              str_detect(pubFundLim, "[Ω]")~-0.25)) %>% #exception:fetal abnormality
